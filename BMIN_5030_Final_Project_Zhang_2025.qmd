  ...
  mutate(
    lead_category = factor(
      ...
      levels = c(
        ...
        "Venous Level >=45",
        "Total" # Added Total to factor levels
      )
    )
  )
...
stats_total <- lead %>%
  ...
  summarise(n = n(), .groups = "drop") %>%
  mutate(lead_category = "Total") # Assign the label
...
leaflet(zip_map) %>%
  addTiles() %>%  # default OSM tiles, free
  addCircleMarkers(
    lng = ~lng, lat = ~lat,
    radius = ~sqrt(n)*2,
...
rf_model <- ranger(
  ...
  importance = "impurity",
  probability = TRUE,
  respect.unordered.factors = "order" # Helps speed up variables with many categories (like Zip)
)
